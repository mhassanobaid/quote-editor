class LineItemsController < ApplicationController
  ...
  def create
    ...
    if @line_item.save
      respond_to do |f|
        f.html { redirect_to quote_path(@quote), notice: "Item was successfully created." }
        f.turbo_stream { flash.now[:notice] = "Item was successfully created." }
      end
    else
  ...
  def update
    if @line_item.update(line_item_params)
      respond_to do |f|
        ...
        f.turbo_stream { flash.now[:notice] = "Item was successfully updated." }
      end
    else
